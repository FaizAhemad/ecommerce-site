...
Current owner override (2026-09-14): do not spend time running format or format:check; the owner will handle formatting later. Continue functional/security implementation, tests, lint/types/offline build and documentation. This supersedes earlier formatting-command requirements.
E26 returns: preserve admin-only minimal history, explicit bounded decision reason, request/order customer consistency and conditional REQUESTED-to-APPROVED/REJECTED updates. No refund or stock mutation is implied. Independent Settings/Messages/Returns panels must not depend on legacy parent load state.

E27 Help/tour: /help is public and the tour starts only on explicit action. Keep steps in memory, retain existing route auth gates, never auto-submit or store customer state, and preserve exit/keyboard controls. Phone/focus/localization acceptance remains pending.

E28 feedback: follow PURCHASE_FEEDBACK.md. Preserve owner-derived first paid order, unique per-user response, private DTOs, bounded inputs and quotas. Purchase-feedback migration is prepared, not applied; feedback errors must not block orders. Feedback is not a public review or incentive. Owner retains production/migration/device acceptance.
