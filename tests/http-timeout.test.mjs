...
test('client long-running requests use the 60 second override', async () => {
  ...
  let scheduledFor = 0
  globalThis.window.setTimeout = (callback, duration) => {
    scheduledFor = duration
    callback()
    return 1
  }
  await assert.rejects(
    apiFetch('/api/upload', { timeoutMs: LONG_RUNNING_API_TIMEOUT_MS }),
    (error) => error instanceof ApiTimeoutError,
  )
...
test('server provider timeout uses the same typed error and override', async () => {
  ...
  try {
    await assert.rejects(
      fetchWithTimeout('https://provider.test', { timeoutMs: LONG_RUNNING_API_TIMEOUT_MS }),
      /upstream request timed out/i,
    )
    assert.equal(scheduledFor, LONG_RUNNING_API_TIMEOUT_MS)
  } finally {
