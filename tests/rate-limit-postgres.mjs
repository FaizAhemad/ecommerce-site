...
// Prisma must initialize after the local environment is loaded.
...
try {
  await db.$transaction(
    async (tx) => {
      ...
      const [before] =
        await tx.$queryRaw`SELECT "expiresAt" FROM "RateLimitBucket" WHERE "key" = 'test-ip'`
  ...
  console.log(
    'PostgreSQL temporary-table checks passed: increments, saturation, isolation, expiry and recovery. Application records unchanged.',
  )
} catch (error) {
  if (error instanceof assert.AssertionError) throw error
  console.error(
    `PostgreSQL check could not complete (${error?.code ?? error?.errorCode ?? 'connection/configuration error'}).`,
  )
  process.exitCode = 1
} finally {
  await db.$disconnect()
