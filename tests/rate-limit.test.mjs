...
const request = (ip = '192.0.2.1', method = 'POST') => ({
  method,
...
test('normal login allowed; eleventh request blocked with private 429 and retry metadata', async () => {
  ...
  for (let i = 0; i < 10; i++)
    assert.equal(
      await enforceRateLimit(request(), response(), 'auth/login', dependencies(store)),
      true,
    )
...
test('window recovers without rejected attempts extending it; different IPs independent', async () => {
  ...
  assert.equal(
    await enforceRateLimit(request('192.0.2.2'), response(), 'auth/login', dependencies(store)),
    true,
  )
  store.advance(1)
  assert.equal(
...
test('multiple limiter invocations share counters; no per-process bypass', async () => {
  ...
  const first = dependencies(store),
    second = dependencies(store)
  const accepted = await Promise.all(
    Array.from({ length: 25 }, (_, i) =>
      enforceRateLimit(request(), response(), 'auth/login', i % 2 ? first : second),
    ),
  )
...
test('verified user quota follows user across IPs and product IDs, not body identifiers', async () => {
  ...
  for (let i = 0; i < 10; i++)
    assert.equal(
      await enforceRateLimit(
        { ...request(`192.0.2.${i + 1}`), body: { userId: `fake-${i}` } },
        response(),
        `products/${i}/reviews`,
        deps,
      ),
  ...
  assert.equal(
    await enforceRateLimit(
      request('192.0.2.99'),
      response(),
      'products/another/reviews/mine',
      deps,
    ),
    false,
  )
  assert.equal(
    await enforceRateLimit(request('192.0.2.99'), response(), 'products/another/reviews', {
      ...deps,
      userId: async () => 'different-user',
    }),
    true,
  )
})
...
test('forged/rotated session cookies cannot bypass the IP ceiling', async () => {
  const store = memoryStore()
  for (let i = 0; i < 60; i++)
    await enforceRateLimit(request(), response(), 'products/p/reviews', dependencies(store))
  const req = request()
  req.headers.cookie = 'gadgify_session=forged-new-token'
  assert.equal(
    await enforceRateLimit(req, response(), 'products/p/reviews', dependencies(store)),
    false,
  )
})
...
test('normal multi-file upload allowed; limit blocks before further identity queries', async () => {
  ...
  for (let i = 0; i < 120; i++)
    assert.equal(await enforceRateLimit(request(), response(), 'admin/upload', deps), true)
...
test('only deployment proxy headers trusted; canonicalizes equivalent addresses', () => {
  assert.equal(
    clientAddress({ ...request('192.0.2.2'), socket: { remoteAddress: '127.0.0.1' } }, false),
    '127.0.0.1',
  )
  ...
  assert.equal(
    clientAddress(request('2001:db8:0:0:0:0:0:1'), true),
    clientAddress(request('2001:db8::1'), true),
  )
...
test('client raises typed 429 once, preserves request, and clears timeout', async () => {
  ...
  globalThis.window = {
    ...
    fetch: async (_url, init) => {
      if (_url === '/api/auth/csrf') return Response.json({ csrfToken: 'a'.repeat(64) })
      calls++
  ...
  try {
    await assert.rejects(
      apiFetch('/api/auth/login', { method: 'POST', body: 'unchanged form' }),
      (error) => error instanceof ApiRateLimitError && error.retryAfterSeconds === 45,
    )
...
test('retry header parsing has safe defaults, bounds and HTTP-date support', () => {
  for (const input of [null, 'nonsense', 'Infinity'])
    assert.equal(new ApiRateLimitError(input).retryAfterSeconds, 60)
  ...
  const seconds = new ApiRateLimitError(new Date(Date.now() + 30000).toUTCString())
    .retryAfterSeconds
