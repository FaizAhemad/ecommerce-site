...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    if (request.method !== 'POST' && request.method !== 'PATCH' && request.method !== 'DELETE')
      return sendError(response, 405, 'METHOD_NOT_ALLOWED', 'Use GET, POST, PATCH, or DELETE.', id)
    ...
    if (request.method === 'DELETE')
      ...
      await db.cartItem.upsert({
        ...
        update: { quantity: request.method === 'POST' ? { increment: quantity } : quantity },
      })
    ...
    return sendError(response, 503, 'DATABASE_UNAVAILABLE', 'Cart is temporarily unavailable.', id)
  }
}
