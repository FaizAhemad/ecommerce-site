...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (!data.startsWith('data:') || data.length > MAX_DATA_URL_LENGTH)
    return sendError(
      response,
      400,
      'VALIDATION_ERROR',
      'Review media must be smaller than 1.5 MB.',
      id,
    )
  ...
  try {
    const blob = await put(`reviews/${randomUUID()}.${media.extension}`, media.bytes, {
      access: 'public',
