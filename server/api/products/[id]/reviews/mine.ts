...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  const media = Array.isArray(body.media)
    ? body.media
        .filter((url): url is string => typeof url === 'string' && isSafeHttpUrl(url))
        .slice(0, 4)
    : []
  if (!Number.isInteger(rating) || rating < 1 || rating > 5)
    return sendError(response, 400, 'VALIDATION_ERROR', 'Rating must be between 1 and 5.', id)
  try {
    const review = await db.$transaction(async (transaction) => {
      ...
      const updated = await transaction.review.update({
        ...
        data: { rating, body: text, media: { create: media.map((url) => ({ url })) } },
        include: { media: true },
      })
      ...
      await transaction.product.update({
        where: { id: productId },
        data: { rating: aggregate._avg.rating ?? rating, reviewCount: aggregate._count.rating },
      })
      return updated
    })
    if (!review)
      return sendError(response, 404, 'NOT_FOUND', 'You have not reviewed this product yet.', id)
    return response.status(200).json({ review, requestId: id })
  } catch {
    return sendError(response, 503, 'DATABASE_UNAVAILABLE', 'Your review could not be updated.', id)
  }
}
