...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    if (request.method !== 'PATCH')
      return sendError(response, 405, 'METHOD_NOT_ALLOWED', 'Use PATCH or DELETE.', id)
    ...
    if (data.name) {
      const duplicate = await db.product.findFirst({
        where: { id: { not: productId }, name: { equals: data.name, mode: 'insensitive' } },
        select: { id: true },
      })
      if (duplicate)
    ...
    if (
      colors &&
      (colors.some((color) => !color.name.trim() || !/^#[0-9a-f]{6}$/i.test(color.hex.trim())) ||
        new Set(colors.map((color) => color.name.trim())).size !== colors.length)
    )
      return sendError(
        response,
        400,
        'VALIDATION_ERROR',
        'Colors need unique names and six-digit hex values.',
        id,
      )
    const mediaImages = Array.isArray(body.images)
      ? body.images.filter(
          (item): item is { url: string; alt?: string; sortOrder?: number } =>
            typeof item === 'object' && item !== null && typeof item.url === 'string',
        )
      : null
    const mediaVideos = Array.isArray(body.videos)
      ? body.videos.filter(
    ...
    const product = await db.$transaction(async (transaction) => {
      const updated = await transaction.product.update({ where: { id: productId }, data })
      if (colors) {
      ...
      if (mediaImages) {
        ...
        await transaction.productImage.createMany({
          data: mediaImages.map((image, index) => ({
            productId,
            url: image.url,
            alt: image.alt ?? updated.name,
            sortOrder: Number.isInteger(image.sortOrder) ? (image.sortOrder as number) : index,
          })),
        })
      }
      if (mediaVideos) {
        await transaction.productVideo.deleteMany({ where: { productId } })
        await transaction.productVideo.createMany({
          data: mediaVideos.map((video, index) => ({
            productId,
            url: video.url,
            poster: video.poster ?? null,
            sortOrder: Number.isInteger(video.sortOrder) ? (video.sortOrder as number) : index,
          })),
        })
      }
      return transaction.product.findUnique({
        where: { id: productId },
        include: { images: true, videos: true, colors: true },
      })
    })
    ...
    return sendError(
      ...
      'Product update is temporarily unavailable.',
      id,
    )
  }
}
