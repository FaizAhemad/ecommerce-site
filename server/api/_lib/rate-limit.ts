...
export function rateLimitRule(path: string, method = 'GET'): Rule | null {
  ...
  if (path.startsWith('auth/')) {
    ...
    if (['auth/signup', 'auth/password-reset-request', 'auth/mobile-request'].includes(path))
      return { scope: 'auth-send', seconds: 600, ip: 5 }
    return { scope: 'auth-verify', seconds: 600, ip: 20 }
  }
  if (method === 'GET') return null
  if (path === 'admin/upload' || /^products\/[^/]+\/review-upload$/.test(path))
    return { scope: 'upload', seconds: 60, ip: 120, user: 40 }
  if (/^products\/[^/]+\/reviews(?:\/mine)?$/.test(path))
    return { scope: 'review', seconds: 60, ip: 60, user: 10 }
  if (path === 'newsletter/subscribe' || path.startsWith('support') || path.startsWith('coupons'))
    return { scope: 'contact-coupon', seconds: 600, ip: 10, user: 5 }
