...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (!/^\+?[1-9]\d{9,14}$/.test(phone))
    return sendError(response, 400, 'VALIDATION_ERROR', 'Enter a valid mobile number.', id)
  try {
    const user = await db.user.findUnique({ where: { phone } })
    if (!user)
      return sendError(response, 404, 'NOT_FOUND', 'No account exists for this mobile number.', id)
