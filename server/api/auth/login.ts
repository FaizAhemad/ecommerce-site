  ...
  try {
    ...
    if (
      ...
      user = await db.user.create({
        data: { email: adminEmail, passwordHash: await hashPassword(adminPassword), role: 'ADMIN' },
      })
    } else if (
      !isPhone &&
      user &&
      ...
      user = await db.user.update({
        where: { id: user.id },
        data: { role: 'ADMIN', passwordHash: await hashPassword(adminPassword) },
      })
    }
    if (!user?.passwordHash || !(await verifyPassword(password, user.passwordHash)))
      return sendError(response, 401, 'UNAUTHORIZED', 'Email or password is incorrect.', id)
