...
| SEC-04 / P0 | [HTTP helper](server/api/_lib/http.ts) sends errors without overriding cache headers; public catalog handlers set public caching before querying. Existing authentication sets HttpOnly/SameSite=Lax cookies, but a complete CSRF/Origin review and consistent dispatcher exception boundary are absent. | Default errors/auth/private responses to no-store; allow public caching only for successful public DTOs. Review unsafe methods, Origin/Fetch Metadata or CSRF tokens as appropriate, preserving signed webhook behavior. Test cookies, cross-origin rejection, expired sessions and malformed routes. |
