...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  const filename = typeof body.filename === 'string' ? body.filename : 'review-media'
  ...
  if (comma < 0) return sendError(response, 400, 'VALIDATION_ERROR', 'The file data is invalid.', id)
  try {
    const blob = await put(`reviews/${Date.now()}-${filename.replace(/[^a-zA-Z0-9._-]/g, '-')}`, Buffer.from(data.slice(comma + 1), 'base64'), { access: 'public', contentType, addRandomSuffix: true })
