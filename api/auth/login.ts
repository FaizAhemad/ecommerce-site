...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const user = isPhone ? await db.user.findUnique({ where: { phone } }) : await db.user.findUnique({ where: { email } })
    ...
    return response.status(200).json({ user: { id: user.id, email: user.email, name: user.name }, requestId: id })
  } catch {
