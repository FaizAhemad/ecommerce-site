# Historical project status — superseded snapshots

Archived during the 2026-09-12 documentation audit to preserve earlier implementation reports and verification notes. This document contains outdated and sometimes contradictory claims. It is not an active checklist or current instruction source. Paths and commands within historical prose may no longer exist.

Use [current status](../../PROJECT_STATUS.md), [the backlog](../../APPLICATION_BACKLOG.md) and [page inventory](../../PAGE_INVENTORY.md) for current implementation, evidence and completion. Original checklist entries below are labelled as historical claims, not present-day completion marks.
...
- Historical implementation claim: Frontend route and responsive UI foundation.
- Historical implementation claim: Initial loading retry state and protected-route demo behavior.
- Historical partial claim: i18n foundation with English, Hindi, and Marathi JSON namespaces; remaining pages still need migration.
- Historical partial claim: Global rendering fallback and storefront retry are in place; per-form/API error handling remains.
- Historical implementation claim: Add Node.js/TypeScript API foundation (see `API_IMPLEMENTATION_PLAN.md`).
- Historical partial claim: Add Prisma schema and PostgreSQL connection; schema is ready, but the production migration waits for deployment credentials.
- Historical implementation claim: Add secure authentication and protected sessions.
- Historical implementation claim: Add products, cart, wishlist, orders, reviews, tracking, and newsletter APIs.
- Historical implementation claim: Add server-side duplicate product-name validation and admin product media upload support.
- Historical implementation claim: Add Razorpay order creation, verification, and webhook handlers.
- Historical partial claim: Connect frontend catalog reads to `/api/products` and cart reads/quantity updates to `/api/cart`; wishlist, checkout, and order screens still need full server synchronization.
- Historical pending claim: Configure Vercel environment variables, migrate Prisma, and deploy.
- Historical pending claim: Add realtime shipment/map tracking after order infrastructure is stable.
...
- Historical partial claim: Added a home newsletter subscription UI and Vercel Resend subscription endpoint. Configure `RESEND_API_KEY`, `RESEND_AUDIENCE_ID`, and optional `RESEND_FROM_EMAIL`; transactional order/auth emails remain backend work.
...
Rate limiting and 429 handling implemented for current write APIs: the single dispatcher uses PostgreSQL atomic counters per trusted client IP and, for protected actions, per database-verified user. The additive RateLimitBucket migration must be applied before deployment. Responses include private cache headers, safe messages, request IDs and Retry-After. Counter failures stop writes with safe 503s. Session restoration/logout, normal GET reads and webhooks are exempt. Client 429s become typed errors, are not automatically retried, preserve failed form state/optimistic rollback, and use translated English/Hindi/Marathi snackbars. `RATE_LIMITING.md` records thresholds, operational cleanup, exclusions and rollout.

Verification: all 20 automated timeout/media/rate-limit tests passed; build/type checks passed and lint completed with existing warnings. Tests use an injected shared counter double for policy scenarios. A separate real PostgreSQL temporary-table test is provided, but could not run here: `.env` and the local Vercel preview environment did not provide a usable DATABASE_URL (Prisma P1012). No database migration was applied and no cleanup job was scheduled. Live database concurrency, authenticated browser 429/recovery and deployment verification remain pending, so the related backlog items remain unchecked. Support/coupon endpoint implementation remains future work.
